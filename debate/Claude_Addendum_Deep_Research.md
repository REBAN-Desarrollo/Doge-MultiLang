...
| **Safety pre-screening** | ElevenLabs Scribe v2 | Whisper + LLM | Scribe v2 tiene entity detection con categoria `offensive_language` nativa; util como STT de safety a costo $0 adicional |
| **Cualquier idioma (90+)** | ElevenLabs Scribe v2 | Whisper large-v3 | Scribe v2: keyterm prompting (100 terminos para nombres de personajes), webhook async, multichannel |
...
### 5.4 Lo que ElevenLabs SI y NO verifica

**ElevenLabs SI provee (actualizado feb 2026):**
- Campo `loss` por caracter y por palabra en Forced Alignment (score de confianza de alineacion)
- Estado del proyecto de dubbing
- **Scribe v2 entity detection:** categorias `pii`, `phi`, `pci`, `offensive_language`, `other` (56 tipos de entidad)
- **Pronunciation Dictionaries:** reglas IPA persistentes por idioma (alias + phoneme), versionadas, CRUD completo
- **Audio Isolation API:** separacion voz/background programatica
- **PVC Speaker Separation:** separar speakers de audio multi-speaker
- **migrate-segments:** reasignacion masiva de speakers a segmentos
...
> **QPH DEBE construir verificacion externa para lo que ElevenLabs no cubre.** Pero ahora hay mas herramientas nativas que aprovechar: entity detection para safety, pronunciation dictionaries para consistencia, y migrate-segments para corregir errores de speaker assignment.
...
| Pronunciation rules | **ElevenLabs Pronunciation Dictionaries** | Incluido en plan | elevenlabs.io (API nativa, reglas IPA, versionadas) |
| Audio pre-processing | **ElevenLabs Audio Isolation** | Incluido en plan | elevenlabs.io (separar voz de background) |
| Speaker correction | **ElevenLabs migrate-segments** | Incluido en plan | elevenlabs.io (reasignacion bulk de speakers) |
| Safety pre-screen | **ElevenLabs Scribe v2 entity detection** | Incluido en plan | elevenlabs.io (offensive_language, pii, phi) |
...
| Pronunciacion | ElevenLabs Pronunciation Dict. | Custom IPA rules + Whisper | Espeak-ng (open source TTS phonemes) |
| Audio isolation | ElevenLabs Audio Isolation | Demucs (open source, Meta) | Adobe Podcast Enhance |
| Speaker correction | ElevenLabs migrate-segments | Manual via Studio UI | pyannote.audio re-diarization |
| Safety screening | ElevenLabs Scribe v2 entity detection | Perspective API (Google) | OpenAI Moderation API |
...
## 10. ALTERNATIVAS DE MODELOS DE AUDITORIA

### 10.1 Para cada capa del pipeline, alternativas evaluadas

La arquitectura de 4 Gates puede implementarse con diferentes combinaciones de herramientas. Aqui las alternativas por capa:

#### Gate 1 (Pre-flight) - Alternativas de Safety Screening

| Opcion | Pros | Contras | Costo | Recomendacion |
|:-------|:-----|:--------|:------|:--------------|
| **A) ElevenLabs Scribe v2 entity detection** | Nativo, sin costo adicional, 56 tipos de entidad | Solo detecta en transcripcion, no en traduccion | $0 | **Usar como primera linea** |
| **B) OpenAI Moderation API** | Robusto, multilingue, categorias granulares | Costo API adicional, latencia | ~$0.001/req | Complemento para Tier 1 |
| **C) Google Perspective API** | Gratis hasta 1QPS, bueno en toxicidad | Menos categorias que OpenAI, lento | $0 (rate limited) | Alternativa gratuita |
| **D) LLM Judge (Claude/GPT-4)** | Mas contexto cultural, entiende matices | Mas caro, mas lento | ~$0.01-0.03/seg | Para casos ambiguos |

**Recomendacion:** A como primera linea (gratis), B o D como segunda linea para contenido flaggeado.

#### Gate 2 (Traduccion EN) - Alternativas de Auditoria de Traduccion

| Opcion | Pros | Contras | Costo | Recomendacion |
|:-------|:-----|:--------|:------|:--------------|
| **A) COMET + 3 LLM Judges** (actual) | Mas robusto, consenso multi-modelo | Mas caro | ~$2-5/ep | **Recomendado para Tier 1** |
| **B) Solo COMET + 1 LLM** | Mas barato, 80% de la precision | Bias de modelo unico | ~$0.50-1/ep | Aceptable para Tier 2 |
| **C) Solo metricas (COMET + chrF++ + BERTScore)** | Gratis, rapido | No detecta errores culturales ni safety | $0 | Solo para Tier 3 |
| **D) Rubric-MQM v2.0 unico** | Auto-post-editing incluido | Depende de un solo framework | ~$1-2/ep | Alternativa a 3 judges |
| **E) Lokalise AI LQA (SaaS)** | Workflow completo, UI, reportes | Vendor lock-in, costo mensual | ~$100-500/mes | Si el equipo no quiere self-host |

**Recomendacion:** A para Tier 1, B para Tier 2, C para Tier 3. D como alternativa si se quiere simplificar.

#### Gate 3 (Multi-idioma) - Alternativas de Verificacion por Idioma

| Opcion | Pros | Contras | Costo | Recomendacion |
|:-------|:-----|:--------|:------|:--------------|
| **A) GEMBA-MQM sin referencia** (actual) | No necesita traduccion de referencia humana | Depende de calidad del LLM | ~$0.01-0.03/seg | **Recomendado** |
| **B) COMET con pseudo-referencia** | Genera referencia via otro LLM, luego compara | Mas complejo, mas caro | ~$0.05/seg | Para idiomas criticos |
| **C) chrF++ solo** | Gratis, bueno para CJK | No detecta errores semanticos | $0 | Baseline minimo |
| **D) Crowdin AI Review** | Integrado con TMS, workflow nativo | Solo si ya usan Crowdin | Variable | Si adoptan Crowdin |

#### Gate 4 (Audio) - Alternativas de Verificacion de Audio

| Opcion | Pros | Contras | Costo | Recomendacion |
|:-------|:-----|:--------|:------|:--------------|
| **A) Whisper + Deepgram dual** (actual) | Alta confianza por consenso | Costo de 2 APIs | ~$3-6/ep | **Recomendado para Tier 1** |
| **B) ElevenLabs Scribe v2 + Whisper** | Reduce a 1 API externa, Scribe incluido en plan | Scribe v2 menos maduro que Whisper en algunos idiomas | ~$1.5-3/ep | **Alternativa costo-eficiente** |
| **C) Solo Whisper** | Mas barato, un solo proveedor | Sin consenso, menor confianza | ~$1.5-3/ep | Aceptable para Tier 3 |
| **D) SenseVoice (CJK) + Whisper (resto)** | Optimo por familia de idioma | Mas complejo de mantener | ~$2-4/ep | Para maximizar precision CJK |

### 10.2 Combinacion Recomendada por Tier

| Tier | Gate 1 (Safety) | Gate 2 (Traduccion) | Gate 3 (Multi-idioma) | Gate 4 (Audio) | Costo estimado |
|:-----|:----------------|:--------------------|:----------------------|:---------------|:---------------|
| **Tier 1** | Scribe v2 + OpenAI Moderation | COMET + 3 LLM Judges | GEMBA-MQM + COMET | Whisper + Deepgram dual | ~$8-13/ep |
| **Tier 2** | Scribe v2 | COMET + 1 LLM | GEMBA-MQM | Scribe v2 + Whisper | ~$3-6/ep |
| **Tier 3** | Scribe v2 | COMET + chrF++ | chrF++ | Solo Whisper | ~$1-3/ep |

> **Nota:** Todos los costos son por episodio (27 idiomas, ~10 min). Los componentes de ElevenLabs (Scribe v2, Pronunciation Dictionaries, Audio Isolation) estan incluidos en el plan Pro sin costo adicional.
