...
| Claude | Forced Alignment NEW (29 idiomas, multilingual v2) | Adoptar para drift detection |
...
| Gemini | `disable_voice_cloning` + `target_accent` (params reales) | Evaluar en spike de API |
...
| Forced Alignment | No existia | ACTIVO, 29 idiomas (multilingual v2) | NUEVO para drift detection |
| from_content_json | No existia | Requiere ventas (bloqueado en Pro) | BLOQUEADO |
| STT Scribe v2 | 29 idiomas (v1) | 90+ idiomas, entity detection, keyterm prompting, offensive_language | Mucho mas potente |
...
=== FASE 3: VALIDACION POST-GENERACION ============================
              ...
              +----------------------+----------------------+
              |                      |                      |
         Whisper STT           Forced Alignment       ElevenLabs Scribe v2
         (OpenAI)              (ElevenLabs, NUEVO)    (entity detection, gratis)
         $0.006/min            29 idiomas              90+ idiomas, offensive_language
              |                      |                      |
              v                      v                      v
...
| STT Scribe v2 | Pre-check + entity detection (offensive_language, keyterm prompting) | PARCIAL | 2h |
| Whisper API (OpenAI) | WER formal post-generacion | NO | 3h |
| GET transcript | Input para validacion | SI (elevenlabs.py) | 0h |
| Pronunciation Dictionaries | Reglas IPA por personaje, versionadas, workspace-shared | NO | 3h |
| Audio Isolation API | Pre-procesar audio de Fernando, remover background | NO | 2h |
| PVC Speaker Separation | Separar speakers de audio multi-speaker programaticamente | NO | 4h |
| migrate-segments | Reasignacion masiva de speakers por segmento | NO | 1h |
| get-similar-voices | Top 10 voces similares a speaker detectado | NO | 1h |

### APIs a NO USAR

| API | Razon |
|:----|:------|
| Manual Dub CSV (mode:manual via API) | "Experimental, production use strongly discouraged" (nota: Manual Dub via Studio UI SI es soportado) |
...
| 2.2 | Dubbing Resource API en elevenlabs.py (incl. migrate-segments, get-similar-voices) | 8-10h |
...
| 2.6 | Pronunciation Dictionaries: reglas IPA por personaje por idioma | 3-4h |
...
| A-020 | Forced Alignment cubre todos los idiomas QPH Tier 3 | **FALSA** | Si: solo 29 idiomas (multilingual v2). Tamil, Filipino, Malay pueden no estar |
| A-021 | `auto_assign_voices: "alpha"` existe como parametro API | **FALSA** | Si: no existe en ningun endpoint documentado (129 docs verificados) |
| A-022 | Scribe v2 con entity detection puede servir como capa de safety | **ALTA (80%)** | Si: `offensive_language` category detecta contenido inapropiado nativamente |
...
| D-004 | Opcion A (Stems) o B (Re-Alignment) o C (PVC API)? | A) Stems B) Re-Alignment C) PVC Speaker Separation API D) Combinacion | Daniel + Fernando | Despues de R-004 |
